...
marker.pos <- x@marker.pos
	...
	totm <- max(rangem)
	tote <- max(rangee)
    plot(markerp[pt.y], genep[pt.x], xlim=c(1, totm), ylim=c(1, tote), xlab="marker position", ylab="expression-trait position",xaxt="n",yaxt="n",pch=16, cex=.5, main = paste("Cutoff =", cutoff, sep = ""))
...
find.cis.genes<-function(out.em, cross, chr.s, marker, marker.pos, exp, exp.pos, thr = thr){
	...
	#if(plotadd & plot){
	#	plot.scanone(out.em, chr = chr.s, main = paste(segdesc[[pheno]], ":Chr ", chr.s, sep = ""), add =T, col = "blue"); abline(v = c(pos.left, pos.right), col = "red")
	#}
	#if(!plotadd & plot){
	#	plot.scanone(out.em, chr = chr.s, main = paste(segdesc[[pheno]], ":Chr ", chr.s, sep = "")); abline(v = c(pos.left, pos.right), col = "red")
	#}
...
#Version: 2.0
#Author: Jeffrey T. Leek <jleek@jhsph.edu>, John D. Storey <jstorey@princeton.edu>, Sangsoon Woo <siriusws@u.washington.edu>
#
...
### Copyright 2007 Jeff Leek and John Storey
    n <- dim(dat)[2]
	m <- dim(dat)[1]
	p <- rep(0,m)
	one <- rep(1,n)
